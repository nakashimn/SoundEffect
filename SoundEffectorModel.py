...
class SoundEffectorModel:
    ...
    def __init__(self):
        ...
        self.BUFFERSIZE = 8*self.CHUNK
        self.MAXLEVEL = 32768.0
        self.HANNINGWINDOW = np.hanning(self.BUFFERSIZE)
        self.HAMMINGWINDOW = np.hamming(self.BUFFERSIZE)

        """ 信号用配列 """
        self.bufferdata = np.empty(self.BUFFERSIZE)
        self.plotdata = np.empty(self.CHUNK)
        self.spectrum = np.empty(self.BUFFERSIZE)
        self.power = np.empty(self.BUFFERSIZE)
        self.cepstrum = np.empty(self.BUFFERSIZE)

        """ 信号処理用変数 """
        self.amp_pre_booster = 6
        self.amp_post_booster = 4
        self.threshold_distortion = 0.2*self.MAXLEVEL
        self.shift_phaser = 0.01
    ...
    def effect(self, input):
        ...
        # 周波数領域で変調処理
        if self.phaser_on == 1:
            effected = self.phaser(effected, amp, phase, self.shift_phaser, self.whole_counter)
...
#        effected = self.ifft(spectrum)
    ...
    def phaser(self, effected, amp, phase, shift, counter):
        original = effected / 2
        phase_shifted = phase + shift*counter*np.linspace(0,phase.shape[0]-1,phase.shape[0])
    ...
    def main(self, stream):
        ...
        """ Plot Data """
        self.bufferdata = self.pushData(processed_data,self.bufferdata)
        self.plotdata = processed_data/self.MAXLEVEL
        self.spectrum = self.fft(self.bufferdata, self.HANNINGWINDOW)
